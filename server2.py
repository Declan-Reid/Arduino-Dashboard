...
async def respond(websocket: WebSocketServerProtocol, path: str):
    ...
    if websocket.request_headers.get('Host') != f'{server_host}:2053':
        print(f"Connection denied: Invalid Host header. Expected {server_host} but got {websocket.request_headers.get('Host')}")
        await websocket.close(code=4000)  # Close the connection with a specific code for "Bad Request"
        return
